...
- **Pin Factory:** It specifically attempts to use `LGPIOFactory`, which is the recommended backend for newer Raspberry Pi hardware (like the Pi 5) to ensure reliable pin control.
- **LED Initialization:** It defines two LED objects:
    - **Read LED:** Connected to **GPIO 20**.
    - **Write LED:** Connected to **GPIO 21**.
- **The `blink` function:** This function triggers a very short pulse (0.01 seconds). It uses `background=True` so that the main monitoring loop isn't paused while the LED is flashing.
...
- **`ctypes` & `libc`:** Since Python doesn't have a built-in high-level wrapper for `fanotify`, the script uses `ctypes` to talk directly to the C standard library (`libc`).
- **Initialization:** The `_initialize_fanotify` method sets up a "fanotify group" and marks a specific mount point (like `/`) to be watched for two types of events:
...
The heart of the script is the `while True` loop inside the `FanotifyMonitor.run` method:
...
2. **Unpacking Metadata:** It uses the `struct` module to "unpack" the binary data into readable Python variables (like the event length, the event mask, and the file descriptor of the file being accessed).
3. **Logic Switch:**
    - If the `mask` contains `FAN_MODIFY`, it calls `blink(write_led)`.
    - If the `mask` contains `FAN_ACCESS`, it calls `blink(read_led)`.
4. **Cleanup:** It immediately closes the file descriptor (`event_fd`) created by the kernel for that specific event to prevent the system from running out of available file handles.

### 4. Command Line Interface
The script uses `argparse` to allow flexibility:
